...
void update_beagle_partials(t_tree* tree, t_edge* b, t_node* d)
{
    ...
    */
    ...
    //Determine d's "left" and "right" neighbors. BTW, we don't need the
...
//    DUMP_I(d->num, n_v1->num, n_v2->num);
//    DUMP_I(b->num,b1->num,b2->num);
    ...
    ret = beagleUpdatePartials(tree->b_inst, operations, 1, 0);
    if(ret<0){
    ...
    ret = beagleGetPartials(tree->b_inst, d->num, 0, (double*)p_lk);
    if(ret<0){
    ...
    int dim1 = tree->mod->ras->n_catg * tree->mod->ns;
