...
/*   rd = Rnorm(post_mean,post_sd); */
  rd = Rnorm_Trunc(post_mean,post_sd,min_r,max_r,&err);
...
/*     } */
  ...
  if(rd < tree->rates->min_rate)      tree->rates->nd_r[d->num] = U1;
  else if(rd > tree->rates->max_rate) tree->rates->nd_r[d->num] = U1;
  else
    {
      if(!d->tax)
	{
