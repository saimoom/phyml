...
public class PhymlPanel extends JPanel implements ActionListener,WindowListener {
	/**
	 * default id
	 */
...
//		tabbedPane.addTab("Trees", treePanel);
		add(tabbedPane);
		nesi_frame = new JFrame(); 
		nesi_frame.addWindowListener(this);
		nesi_frame.setTitle("NeSI");
		nesi_frame.setLocation(100, 100);
		nesi_frame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		nesi_frame.setResizable(true);
		Container visibleArea = nesi_frame.getContentPane();
		visibleArea.add(gridPanel);
		gridPanel.setPreferredSize(new Dimension(650, 650));
		nesi_frame.pack();
		gridPanel.requestFocusInWindow();
		nesi_frame.setVisible(false);
	}
