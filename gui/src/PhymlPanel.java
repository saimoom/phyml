...
//		tabbedPane.addTab("Trees", treePanel);
	...
	public void actionPerformed(ActionEvent a) {
		if (iDP.getIsFile() && !iDP.getInputPath().equals("")
				&& (new File(iDP.getInputPath())).exists()) {
			tabbedPane.setSelectedIndex(1);
			callPHYML.callPhyML(iDP.getInputPath(),
				/**
				 * -i (or --input) seq file name seq file name is the name of the
				 * nucleotide or amino-acid sequence file in PHYLIP format.
				 */
				iDP.getDataType(),
				/**
				 * -d (or --datatype) data type data type is nt for nucleotide
				 * (default) and aa for amino-acid sequences.
				 */
				iDP.getFileFormat(),
				/**
				 * -q (or --sequential) Changes interleaved format (default) to
				 * sequential format.
				 */
				iDP.getNumDataSets(),
				/**
				 * -n (or --multiple) nb data sets nb data sets is an integer giving
				 * the number of data sets to analyse.
				 */
				tS.getStartTree(),// TODO check this method
				/**
				 * -p (or --pars)Use a minimum parsimony starting tree. This
				 * option is taken into account when the ’-u’ option is
				 * absent and when tree topology modifications are to be
				 * done.
				 */
				bS.getNumBootstrap(),// TODO check this method
				/**
				 * -b (or --bootstrap) int – int > 0: int is the number of
				 * bootstrap replicates. – int = 0: neither approximate
				 * likelihood ratio test nor bootstrap values are computed.
				 * – int = -1: approximate likelihood ratio test returning
				 * aLRT statistics. – int = -2: approximate likelihood ratio
				 * test returning Chi2-based para-metric branch supports. –
				 * int = -4: SH-like branch supports alone.
				 */
				sM.getSubModel(),
				/**
				 * -m (or --model) model name model name : substitution
				 * model name. – Nucleotide-based models: HKY85 (default) |
				 * JC69 | K80 | F81 | F84 | TN93 | GTR | custom The custom
				 * option can be used to define a new substitution model. A
				 * string of six digits identifies the model. For instance,
				 * 000000 corresponds to F81 (or JC69 provided the
				 * distribution of nucleotide frequencies is uni-form).
				 * 012345 corresponds to GTR. This option can be used for
				 * encoding any model that is a nested within GTR. See
				 * Section 6.1.2. NOTE: the substitution parameters of the
				 * custom model will be optimised so as to maximise the
				 * likelihood. It is possible to specify and fix (i.e.,
				 * avoid optimisation) the values of the substitution rates
				 * only through the PHYLIP-like interface. – Amino-acid
				 * based models: LG (default) WAG | JTT | MtREV | Dayhoff |
				 * DCMut | RtREV | CpREV | VT | Blosum62 | MtMam | MtArt |
				 * HIVw | HIVb | custom The custom option is useful when one
				 * wants to use an amino-acid substitution model that is not
				 * available by default in PhyML. The symmetric part of the
				 * rate matrix, as well as the equilibrium amino-acid
				 * frequencies, are given in a file which name is asked for
				 * by the program. The format of this file is described in
				 * section 7.4.
				 */
				sM.getAARateFileName(),
				/**
				 * --aa rate file file name This option is compulsory when
				 * analysing amino-acid sequences under a ‘custom’ model.
				 * file name should provide a rate matrix and equilibrium
				 * amino acid in PAML format (see Section ).
				 */
				sM.getFrequencies(),
				/**
				 ...
				 * (Empirical) the equilibrium amino-acid frequencies are
				 * estimated by counting the occurence of the different
				 * amino-acids in the alignment. – m : the character
				 * frequencies are determined as follows : - Nucleotide
				 * sequences: (ML) the equilibrium base frequencies are
				 * estimated using maximum likelihood. - Amino-acid
				 * sequences: (Model) the equilibrium amino-acid frequencies
				 * are estimated using the frequencies defined by the
				 * substitution model. – “fA,fC,fG,fT” : only valid for
				 * nucleotide-based models. fA, fC, fG and fT are floating
				 * numbers that correspond to the frequencies of A, C, G and
				 * T respectively.
				 */
				sM.getTsTvRatio(),
				/**
				 * -t (or --ts/tv) ts/tv ratio ts/tv ratio:
				 * transition/transversion ratio. DNA sequences only. Can be
				 * a fixed positive value (e.g., 4.0) or type e to get the
				 * maximum likelihood estimate.
				 */
				sM.getPropInvarSites(),
				/**
				 * -v (or --pinv) prop invar prop invar: proportion of
				 * invariable sites. Can be a fixed value in the [0,1] range
				 * or type e to get the maximum likelihood estimate.
				 */
				sM.getNumSubCat(),
				/**
				 * -c (or --nclasses) nb subst cat nb subst cat: number of
				 * relative substitution rate categories. Default: nb subst
				 * cat=4. Must be a positive integer.
				 */
				sM.getAlpha(),
				/**
				 * -a (or --alpha) gamma gamma: value of the gamma shape
				 * parameter. Can be a fixed positive value or e to get the
				 * maximum likelihood estimate. The value of this parameter
				 * is estimated in the maximum likelihood framework by
				 * default.
				 */
				sM.getUseMedian(),
				/**
				 * --use median The middle of each substitution rate class
				 * in the discrete gamma distribution is taken as the
				 * median. The mean is used by default.
				 */
				"",// TODO check with Stephane
				/**
				 * --free rates As an alternative to the discrete gamma
				 * model, it is possible to estimate the (relative) rate in
				 * each class of the (mixture) model and the corresponding
				 * frequencies. This model has more parameters than the
				 * discrete gamma one but usually provides a significantly
				 * better fit to the data.
				 */
				"",// TODO check with Stephane
				/**
				 * --codpos 1,2 or 3 When analysing an alignment of coding
				 * sequences, use this option to consider only the first,
				 * second or third coding position for the estimation.
				 */
				tS.getSearch(),
				/**
				 * -s (or --search) move Tree topology search operation
				 * option. Can be either NNI (default, fast) or SPR (a bit
				 * slower than NNI) or BEST (best of NNI and SPR search).
				 */
				 ...
				 * optimised. – params=tl: tree topology and branch lengths
				 * are optimised. – params=lr: branch lengths and
				 * substitution rate parameters are optimised. – params=l:
				 * branch lengths are optimised. – params=r: substitution
				 * rate parameters are optimised. – params=n: no parameter
				 * is optimised.
				 */
				tS.getRandStart(),
				/**
				 * --rand start This option sets the initial tree to random.
				 * It is only valid if SPR searches are to be performed.
				 */
				tS.getNumRandStart(),
				/**
				 * --n rand starts num num is the number of initial random
				 * trees to be used. It is only valid if SPR searches are to
				 * be performed.
				 */
				"",// TODO randSeed (might generate a random number here)
				/**
				 * --r seed num num is the seed used to initiate the random
				 * number generator. Must be an integer.
				 */
				"yes",// TODO printSitelnl (for now set to yes)
				/**
				 * --print site lnl Print the likelihood for each site in
				 * file * phyml lk.txt.
				 */
				"yes",// TODO printTrace (for now set to yes)
				/**
				 * --print trace Print each phylogeny explored during the
				 * tree search process in file * phyml trace.txt.
				 */
				iDP.getRunID(),
				/**
				 * --run id ID string Append the string ID string at the end
				 * of each PhyML output file. This optionmay be useful when
				 * running simulations involving PhyML. It can also be used
				 * to ‘tag’ multiple analysis of the same data set with
				 * various program settings.
				 */
				"yes",// TODO noMemoryCheck (for now set to yes later on we
						// could add a warning and let the user decide)
				/**
				 * --no memory check By default, when processing a large
				 * data set, PhyML will pause and ask the user to confirm
				 * that she/he wants to continue with the execution of the
				 * analysis despite the large amount of memory required. The
				 * --no memory check skips this question. It is especially
				 * useful when running PhyML in batch mode.
				 */
				"",// TODO noJcolAlias (for now we want it faster)
				/**
				 * -no jcolalias By default, PhyML preprocesses each
				 * alignment by putting together (or aliasing) the columns
				 * that are identical. Use this option to skip this step but
				 * be aware that the analysis might then take more time to
				 * complete.
				 */
				"",// TODO contrainedLens for now empty need to check with
					// Stephane
				/**
				 * --contrained lens When an input tree with branch lengths
				 * is provided, this option will find the branch multiplier
				 * that maximises the likelihood (i.e., the relative branch
			...
			try {
				callPHYML.callPhyML(
					UnpackTestDataFile.start(iDP.getInputPath()),
					/**
